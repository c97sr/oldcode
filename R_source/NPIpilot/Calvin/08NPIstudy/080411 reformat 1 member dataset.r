...
demographic_entry <- c( "q2_dob_m", "q2_sex_m", "q2_occ_m", "q2_edu_m","q2_smoke_m","q2_smokec_m","q2_smokeq_m",
"q2_smokenum_m","q2_ill_m","q2_whichill_m", "q2_work_m", "q2_indoor_m", "q2_outdoor_m", "q2_indtype_m", "q2_wkcontact_m", 
"q2_sch_m", "q2_schtype_m","q2_class_m")
...
# adding the chronic disease boolean entries (some problems in the program
...
demographic_output$what_disease <- as.character(demographic_output$what_disease)
demographic_output$what_disease[is.na(demographic_output$what_disease) | demographic_output$what_disease =="" ] <- NA
#demographic_output$what_disease[demographic_output$what_disease=="nose sens"] <-NA
...
demographic_output$dob <- dates(as.character(demographic_output$dob), format= "d/m/y")
#demographic_output$age <-round((dates("07-07-01", format= "y-m-d") - dates(demographic_output$dob))/365,0)
hhsize <- hh$q2_hhsize              # family size
visitdate <- hh$q2_date        # date of first home visit
demographic_output$visitdate <- rep(hh$q2_date,hh$q2_hhsize)
demographic_output$visitdate <- dates(as.character(demographic_output$visitdate), format= "d/m/y")
...
#sort by hhID
demographic_output$hhID <- as.numeric(as.character(demographic_output$hhID))
...
write.csv(demographic_output, paste(output_dir, today,"_demographic_m.csv", sep=""), row.names=FALSE)
...
#sort by hhID
baseflu_output$hhID <- as.numeric(as.character(baseflu_output$hhID))
baseflu_output<-baseflu_output[order(baseflu_output$hhID),]

write.csv(baseflu_output, paste(output_dir, today,"_baseflu_m.csv", sep=""), row.names=FALSE)

#####################
#3. 080513 get ventilation data

vent_name <- c("window","fan")
vent_entry <- c("q4_openwd_m","q4_fan_m")
vent_output <-getallmember_thing(hh, vent_name, vent_entry) 

vent_output$hhID <- as.numeric(as.character(vent_output$hhID))
vent_output<-vent_output[order(vent_output$hhID),]

write.csv(vent_output, paste(output_dir, today,"_vent_m.csv", sep=""), row.names=FALSE)

#old data in 2007, but modified in 2008
...
#write.csv(followup_output, paste(output_dir, today,"_followup_m.csv", sep=""), row.names=FALSE)
...
write.csv(aftervisit_output, paste(output_dir, today,"_aftervisit_m.csv", sep=""), row.names=FALSE)
...
write.csv(maskcom_output, paste("P:\\GROUP\\CDC\\NPI data\\", today,"_maskcom_m.csv", sep=""), row.names=FALSE)
...
write.csv(handcom_output, paste("P:\\GROUP\\CDC\\NPI data\\", today,"_handcom_m.csv", sep=""), row.names=FALSE)
